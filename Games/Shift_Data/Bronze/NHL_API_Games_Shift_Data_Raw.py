    ...
    return spark.sql(f"""
                ...
                games as (
                    ...
                    select /*+ broadcast (p) */ distinct
                        ...
                        where 1 = 1
                            ---NHL didn't start tracking shift data until 20102011 season, only going after regular season and playoff games
                    ...
                    games_in_play as (
                        ...
                        select /*+ broadcast (p) */ distinct
                                ...
                                start_time_utc
                        from games a
                        left anti join games_ended b 
                            on a.game_id = b.game_id 
                        cross join date_param p
                        where 1 = 1
                            and game_date <= p.current_run_date
                            ---below looks to see if the current timestamp is at least 15 minutes after the game start time since games typically have a delayed start time
                    ...
                    games_not_loaded as (
                        ...
                        left anti join games_loaded b
                            on a.game_id = b.game_id
                        ---take out games where the retry_scrape_ind = false 
...
if kickoff:
    ...
    while True: 
        ...
        final_pass = buckets.issubset({"in play", "last two", "ended today", "loaded but missing data"})
