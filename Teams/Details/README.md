...
The Team Details section also incorporates `nhl_data_staged.teams.master_ids` to include teams that were not Original Six or part of an Expansion era (e.g. Hamilton Tigers, Toronto St. Patricks). The endpoint may include values for `last_active_season` if the franchise is no longer active, however if a franchise relocated to a new city 
and assumed a new team name, then the `nhl_data_staged.games.schedules` table is used to derive the franchise's current team `active_from`, `active_to`, `is_active`, and `last_active_season` fields.
...
- `nhl_data_staged.games.schedules` (silver): used to derive dates that a team was `active_from` and `active_to`, but will also derive `last_active_season`, and `is_active` fields in the event the payload from `nhl_data_raw.teams.details` doesn't contain this information
- `nhl_data_staged.teams.details` (silver): cleanses and transforms data from `nhl_data_raw.teams.details` to track whether a team is active or not, including fields
  such as `active_from`, `active_to`, `is_active`, and `last_active_season`
