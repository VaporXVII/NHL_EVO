...
import requests, json, time, random, threading, string, datetime, concurrent.futures, math
...
def insert_data(source: DataFrame, target_table: str, sample_size: int) -> None:
    ...
    spark.sql(f"""
                with src as (
                    ...
                    from source_tmp 
                    where 1 = 1
                        and size(from_json(payload, '{json_schema}').data) > 0
                )
                merge into {target_table} t   
                using src s  
                    on t.request_key = s.request_key 
                    and t.api_url = s.api_url 
                    and from_utc_timestamp(t.ingest_ts_utc, '{user_region}')::date = from_utc_timestamp(current_timestamp(), '{user_region}')::date
...
season_search = spark.sql(f"""
                ...
                current_season_dates as (
                    ...
                    ---NHL released pre-season schedule THEN regular season schedule later
                    ---by setting a.game_date >= p.current_run_dte, this will ensure that new player info is collected for 
                    ---players that have swapped teams in the offseason, but then after the preseason is over only the 
                    ---regular season and playoff games will run
                    ...
                    union all 
                    ---separate regular season game dates and playoff game dates as the former might not yet be loaded
                    select /*+ broadcast (p) */ distinct 
                        a.season, 
                        a.game_date, 
                        a.game_type
                ...
                last_active_season as (
                    ...
                    select 
                        coalesce(max(a.last_active_season), 19001901)::integer as last_active_season_players_table
                    from nhl_data_staged.players.master_ids a
                    where 1 = 1
                        and a.last_active_season is not null 
                ...
                select /*+ broadcast (b) */
                    a.start_season,
                    a.end_season as last_active_season_sched_table,
                    b.last_active_season_players_table,
                    a.current_season,
                    a.game_date,
                    a.game_type, 
                    a.date_idx,
...
if ready:
    ...
    with ThreadPoolExecutor(max_workers = 5) as executor:
        ...
        api_data = []
        futures = [executor.submit(call_api, url = url) for url in player_urls]
        futures = [
            executor.submit(
                call_api,
                url,
        ...
        if insert_ready: 
            ...
            insert_data(source = players_df, target_table = "nhl_data_raw.players.player_search", sample_size = row_sample)
            run_table_maint(spark, "nhl_data_raw.players.player_search")
...
if ready:
    ...
    if not players_season.isEmpty():
        #table below is source that feeds nhl_data_staged.players.master_ids
        insert_data(source = players_season, target_table = "nhl_data_raw.players.player_search_season", sample_size = row_sample)
        run_table_maint(spark, "nhl_data_raw.players.player_search_season")
else: 
    print(f"Current season still in play, skipping player ids scrape.")
